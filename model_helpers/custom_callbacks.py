'''
Callbacks written for Keras models to calculate and record a model's 
precision, recall, and f-score during training.
'''

import numpy as np
from tensorflow.keras.callbacks import Callback
from analysis import classifier_metrics as cm 
...
class BinaryClassCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        prec = cm.calculate_class_precision(1, preds, labels)
        rec = cm.calculate_class_recall(1, preds, labels)
...
class BinaryClassMultiInputOutputCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        prec_main = cm.calculate_class_precision(1, preds_main, labels)
        rec_main = cm.calculate_class_recall(1, preds_main, labels)
        fscore_main = cm.calculate_class_fscore(prec_main, rec_main)

        prec_aux = cm.calculate_class_precision(1, preds_aux, labels)
        rec_aux = cm.calculate_class_recall(1, preds_aux, labels)
        fscore_aux = cm.calculate_class_fscore(prec_aux, rec_aux)
